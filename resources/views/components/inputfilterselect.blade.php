    ...
    >
        ...
        @if(isset($options))
            @foreach($options as $index => $value)
                <option value="{{$value[$key ?? 'kelomp']}}" {{ isset($disabledOption) && $disabledOption == $value[$key ?? 'id'] ? 'disabled' : '' }}>{{$value[isset($labelKode) ?  $labelKode : 'kode_kelompok']}} - <i>{{$value[isset($labelKey) ?  $labelKey : 'nama_item']}}</i></option>
            @endforeach
        @endif
    </select>
</div>
