            ...
            <div class="panel-body panel-form col-lg-4">
                <div class="container-fluid col-lg-12">        
                    <div class="ml-5">
                        ...
                        @foreach ($pelatihan_all as $item)
                            @if ($item->id == $pelatihan->id)
                            @else
                            <div class="row col-lg-10 border mb-3 p-10" style="min-height: 100px;">                                
                                <p class="col-sm-12">{{$item->created_at}}</p>
                                <a href="{{url('detail_pelatihan_user')}}/{{$item->id}}" class="text-dark"><h6 class="col-sm-12">{{$item->judul_pelatihan}}</h6></a>
